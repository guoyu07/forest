...
if ( !function_exists( 'madeleine_default_popularity_options' ) ) {
  function madeleine_default_popularity_options() {
    $defaults = array(
      'popularity_status' => 0
    );
    return apply_filters( 'madeleine_default_popularity_options', $defaults );
  }
}
...
if ( !function_exists( 'madeleine_initialize_popularity_options' ) ) {
  function madeleine_initialize_popularity_options() {
    if( false == get_option( 'madeleine_popularity_options' ) )
      add_option( 'madeleine_popularity_options', apply_filters( 'madeleine_default_popularity_options', madeleine_default_popularity_options() ) );
...
if ( !function_exists( 'madeleine_popularity_callback' ) ) {
  function madeleine_popularity_callback() {
    echo '<p>The Popularity Plugin is a feature built in the Madeleine Theme. It retrieves the <strong>share count</strong> from the following sites:</p>';
    echo '<ul>';
    ...
    endforeach;
    echo '</ul>';
    echo '<p>Whenever you create a new standard post, the plugin will set up a <strong>scheduled event</strong> to retrieve its share count <strong>once per day</strong>.<br>
    It will then sum up all these individual share counts into a <strong>total</strong> share count.</p>';
    echo '<p>This total share count is used in the Popularity widget (available in the <a href="' . get_admin_url() . 'widgets.php">Widgets panel</a>). This widget displays the <strong>most shared posts in the last 30 days</strong>.</p>';
    echo '<h3>Cron events</h3>';
    echo '<p>Because the Popularity plugin makes use of the <a href="http://codex.wordpress.org/Category:WP-Cron_Functions">WordPress Cron functions</a>, it will populate scheduled events for each post that you publish.<br>
    ...
    echo '<h3>Status</h3>';
    echo '<p>By default, the Popularity plugin is disabled. Just select "Enabled" and save your changes to enable the plugin. From that moment on, it will schedule daily events to retrieve the share counts.</p>';
  }
}
...
if ( !function_exists( 'madeleine_popularity_status_callback' ) ) {
  function madeleine_popularity_status_callback( $args ) {
    $settings = get_option( 'madeleine_popularity_options' );
